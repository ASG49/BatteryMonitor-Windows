//=============================================================================
// BatteryMonitor.cs - Windows Battery Monitor v34.6
...
namespace BatteryMonitor
{
    ...
    {
        private const string AppVersion = "v34.6";  // ← UPDATE ONLY HERE
        ...
        public MainForm()
        {
            ...
            // Use GetDC(desktop) — reliable before form handle exists, works on Win7/10/11.
        ...
        private void BuildUI()
        {
            ...
            // Create header font for measurement
            int hdrFontSize = (dpiScale <= 1.0f) ? 20 : 23;
            _headerFontRef = new Font("Segoe UI", SF(hdrFontSize), FontStyle.Bold);

            // Column widths measured from widest realistic text using actual header font
            int[] colW = new int[] {
                HW("DCHRG"),          // 0: Status
                HW("V=-19.99 V"),     // 1: Voltage
                HW("i=-3.00 A"),      // 2: Current
                HW("R=999.9 Ω"),      // 3: Resistance
                HW("P=-99.9 W"),      // 4: Power
                HW("Qin=99.99 Ah"),   // 5: Qc / Qin
                HW("Ein=999.99 Wh"),  // 6: Ec / Ein
                HW("Qt=99.99 Ah"),    // 7: Qt / η
                HW("Et=999.99 Wh"),   // 8: Et / T=
                HW("SOC=100.0 %") + S(12), // 9: SOC
            };
            ...
            tbl.RowStyles.Add(new RowStyle(SizeType.Percent, 50));
            for (int c = 0; c < 10; c++)
                tbl.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, colW[c]));

            // Row 0: BMS
            tbl.Controls.Add(lblStatus,     0, 0);
            tbl.Controls.Add(lblVoltage,    1, 0);
            tbl.Controls.Add(lblCurrent,    2, 0);
            tbl.Controls.Add(lblResistance, 3, 0);
            tbl.Controls.Add(lblPower,      4, 0);
            tbl.Controls.Add(lblSegCap,     5, 0);
            tbl.Controls.Add(lblSegEnergy,  6, 0);
            tbl.Controls.Add(lblTotCap,     7, 0);
            tbl.Controls.Add(lblTotEnergy,  8, 0);
            tbl.Controls.Add(lblSOC,        9, 0);

            // Row 1: TC66
            tbl.Controls.Add(lblTC66Status,     0, 1);
            tbl.Controls.Add(lblTC66Volt,       1, 1);
            tbl.Controls.Add(lblTC66Amp,        2, 1);
            tbl.Controls.Add(lblTC66Resistance, 3, 1);
            tbl.Controls.Add(lblTC66Power,      4, 1);
            tbl.Controls.Add(lblTC66mAh,        5, 1);
            tbl.Controls.Add(lblTC66mWh,        6, 1);
            tbl.Controls.Add(lblTC66Eff,        7, 1);
            tbl.Controls.Add(lblTC66Temp,       8, 1);
            ...
            // TC66 row initially hidden until connected
            ...
            this.Controls.Add(tbl);   // Two-row header table (BMS + TC66)
        }
        ...
        private Label MakeHeaderLabel(string text, Color color)
        {
            ...
            int baseFontSize = (dpiScale <= 1.0f) ? 20 : 23;
    ...
    static class Program
    {
        ...
        private static extern int SetProcessDpiAwareness(int awareness);  // 2 = PerMonitorDpiAware

        [STAThread]
        static void Main()
        {
            // Must be called before ANY window or graphics context is created.
            // Try Per-Monitor DPI awareness (Win 8.1+) first, fall back to system-aware (Win 7/10).
