//=============================================================================
// BatteryMonitor.cs - Windows Battery Monitor v46.0
// May 25, 2025 - Tony Gozdz
//
...
//   v46.0 (Jun 10, 2026): LTTB decimation (10,000 pts) on all draw methods — fixes UI hang on long sessions
//   v45.9 (Jun 09, 2026): Low-SOC speech: Math.Ceiling for 0.1% SOC resolution — spoken value matches header display
//   v45.8 (Jun 08, 2026): Auto-stop charge alert on Idle→Discharging; dismiss button label ✕ Beep → 🔕
//   v45.7 (Jun 07, 2026): Spread green/teal spectrum; SOC discharge → pure red; darken current greens
//   v45.6 (Jun 07, 2026): Header Qc/Ec/Qt/Et label colors now match chart curve colors (teal/purple)
//   v45.5 (Jun 07, 2026): Axis title fixes: full "Name, Unit" format; font 22→20px; per-chart MarginLeft/MarginRight
//   v45.4 (Jun 07, 2026): Y-axis labels: full words — Voltage/Current/Power/Energy/Capacity/Temperature
//   v45.3 (Jun 07, 2026): CSV load: set CurrentBatteryState from last row — axis/tick colors now sync on file review
//   v45.2 (Jun 07, 2026): Fmt3() helper — consistent 3 sig figs for P/W/Wh/Ah everywhere (header, badges, tooltip)
//   v45.1 (Jun 06, 2026): Replace beeps with speech — "Battery full" at charge complete; "ten".."one" at low SOC
//   v45.0 (Jun 02, 2026): Adaptive format for W/Wh: F2 when <10, F1 when >=10; current/voltage/Ah always F2
//   v44.9 (Jun 02, 2026): All non-resistance values → F2 (max 2 decimals); G3 removed
//   v44.8 (Jun 02, 2026): Current i= F2 (2 decimals); resistance R= F0 (integer); rest keeps G3
//   v44.7 (Jun 02, 2026): G3 format (3 sig figs) for all non-voltage values; voltage keeps F2
//   v44.6 (May 29, 2026): ETA label renamed to TT5 (Time to 5% SOC)
//   v44.5 (May 29, 2026): Dispose beepTimer in OnFormClosing — alert could persist after StopRecording
//   v44.4 (May 29, 2026): Charge-complete triple-beep alert (10 min, ✕ button); low-SOC single beep at 10%→1% during discharge
//   v44.3 (May 29, 2026): Removed dead methods GetVoltageFromWMI, GetBatteryTemperatureFromWMI, WriteThermalDiagnostics; removed Diag button
//   v44.2 (May 29, 2026): Fix stuck BMS current — DischargeRate=0 (non-null) blocked ChargeRate read; now checked independently
//   v44.1 (May 29, 2026): Fix CE=-- on mid-charge start — call ResetEfficiencyBaseline on Idle→Charging transition
//   v44.0 (May 29, 2026): Fix Qc/Ec reset bug — segment only resets on Charging↔Discharging, not on Idle transitions
//   v43.9 (May 29, 2026): Power badge format F1→G3 (3 significant figures)
//   v43.8 (May 29, 2026): Col 7 (Qt/CE) 11→10%, col 8 (Et) 9→10%
//   v43.7 (May 29, 2026): Header label margin → Padding.Empty (zero padding)
//   v43.6 (May 29, 2026): Fix AdjustHeaderFontSize — compare PreferredWidth vs column pixel width (AutoSize bug); col 0 5→6%, col 7 12→11%
//   v43.5 (May 29, 2026): AdjustHeaderFontSize rewritten — apply+layout+PreferredWidth check; no GDI measurement
//   v43.4 (May 29, 2026): AdjustHeaderFontSize: reset to 20pt each call so font grows back on wider screens
//   v43.3 (May 29, 2026): Header label margin: right S(8)→S(4), top S(4)→S(2) — less padding between cells
//   v43.2 (May 29, 2026): AdjustHeaderFontSize() — auto-shrinks header labels 20→14pt when content clips on narrow screens
//   v43.1 (May 27, 2025): Legend labels: removed space after comma in ", unit" style
//   v43.1 (May 27, 2025): V→U, η→CE, Ttc→T.TC, H→SoH; TC66 legend .TC suffix; badge removes "T="; spaces in all legend units
//   v43.0 (May 27, 2025): Legend/axis labels: parentheses → ", unit" style; TC66 temp "T.TC, °C"
//   v42.9 (May 27, 2025): Fix Ttc label blinking — remove conflicting lblBMSTemp.Visible=false from UpdateHeader
//   v42.8 (May 27, 2025): Cleanup — removed dead BMS temp chart infrastructure (xDataBMSTemp, BMSTempColor, AddBMSTempPoint, tooltip, legend, scoring); removed HasTemperature property; removed clockTickCount temp polling; fixed stale header comments
//   v42.7 (May 27, 2025): TC66 temp curve thickness 2.5f→0.5f to match other TC66 curves
//   v42.6 (May 27, 2025): Removed ThermalZones.txt diagnostic dump
//   v42.5 (May 27, 2025): TC66 temp label "| °C" → "Ttc=°C"
//   v42.4 (May 27, 2025): TC66 temp split into separate red label (col 8,1); η label shows efficiency only
//   v42.3 (May 27, 2025): TempColor → Color.Red (TC66 temp curve bright red)
//   v42.2 (May 27, 2025): T(BMS) header removed; chart T curve reverted to TC66 temp; legend "T.TC, °C"
//   v42.1 (May 27, 2025): Restore 80°C upper limit — Yoga 7 has only THM0 (CPU/platform, ~81°C); T(BMS) correctly hidden
//   v42.0 (May 27, 2025): Thermal zone diagnostic dump → ThermalZones.txt on first read
//   v41.9 (May 27, 2025): T(BMS) thermal zone: prefer BAT-named zone; fallback to minimum (was: last zone = CPU temp)
//   v41.8 (May 27, 2025): BMS temp polled every 10s from OnClockTick — no longer stale when not recording
//   v41.7 (May 27, 2025): η label separator: space→" | " before TC66 temp
//   v41.6 (May 27, 2025): HasTemperature property: =>  syntax → get{} for .NET 4.0 compat
//   v41.5 (May 27, 2025): Fix CS0103 — expose BatteryReader.HasTemperature property; use reader.HasTemperature in MainForm
//   v41.4 (May 27, 2025): col 10 (H/Cyc) 13%→9%; col 7 (η) 9%→12%; col 8 8%→9%
//   v41.3 (May 27, 2025): TC66 temp appended to η label e.g. η=68.5% 35°C
//   v41.2 (May 27, 2025): T(BMS) keeps row 1 open regardless of TC66/ETA state; row 1 expands on first BMS temp
//   v41.1 (May 27, 2025): BMS temp curve thickness 3.5f→2.5f
//   v41.0 (May 27, 2025): T(BMS) in header always visible at col 8,1; removed dead TC66 temp callout block; removed unused lastPlottedBMSTemp field
//   v40.9 (May 27, 2025): Legend reordered SOC first; TC66 temp empty-if fixed; BMS temp always plotted
//   v40.8 (May 27, 2025): BMS temp toast moved to S(38); curve threshold 2→1 point; lastPlottedBMSTemp reset on Clear
//   v40.7 (May 27, 2025): TC66 temp removed from chart; BMS temp uses red line; TC66 temp shown in header only
//   v40.6 (May 27, 2025): BMS temp plotted only on value change — eliminates flat staircase; sparse stepped curve shows real updates
//   v40.5 (May 26, 2025): η and TC66T separated into cols 7/8; BMS temp col 9 yellow; H= prefix; columns rebalanced
//   v40.4 (May 26, 2025): Col widths rebalanced: col 7 (η+T) 13%, col 10 (Hlt) 14%
//   v40.3 (May 26, 2025): TC66 temp prefix T= added; Hlt→H= for column fit
//   v40.2 (May 26, 2025): TC66 temp appended to η label; BMS temp col 8 row 1 yellow; Hlt= prefix; lblTC66Temp removed from table
//   v40.1 (May 26, 2025): BMS temp label fixed — TC66 temp labeled USB:; Hlth prefix shortened to H= to fit %
//   v40.0 (May 26, 2025): Col 10 narrowed to 8%; data columns restored to original widths
//   v39.9 (May 26, 2025): Column percentages corrected to sum to 100%; redistributed to restore units in all columns
//   v39.8 (May 26, 2025): Col 10 widened 10→13% so Hlth=96.8% % is not clipped
//   v39.7 (May 26, 2025): GetAllWMIData() consolidates all WMI queries; Health/Cycles in new col 10; DirectPower from BatteryStatus; yellow ETA background
//   v39.6 (May 26, 2025): lblTC66Temp hidden in initial hide list — removes T=--°C ghost in row 2
//   v39.5 (May 26, 2025): BMS temp always shown in header; TC66 temp appended after BMS temp when TC66 connected; lblTC66Temp hidden
//   v39.4 (May 26, 2025): BMS battery temperature displayed in header row 1 col 8 when TC66 not connected (requires admin manifest)
//   v39.3 (May 26, 2025): Fixed battery temperature — removed BAT* pattern filter; TZ01 now returned correctly
//   v39.2 (May 26, 2025): SOC 110% ceiling only when data >= 90%; normal tight padding below 90%
//   v39.1 (May 26, 2025): SOC Y2: fixed 110% ceiling (10% headroom above 100%); MaxTicks 6→8 for 5-7 visible ticks
//   v39.0 (May 26, 2025): Auto-LR line always extends to left axis edge via regression extrapolation
//   v38.9 (May 25, 2025): Auto-LR left anchor clamped to visible chart area (Math.Max with xFirst)
//   v38.8 (May 25, 2025): Auto-LR uses last 5-minute window of SOC points; fallback to N most recent if fewer than N in window
//   v38.7 (May 25, 2025): Chart margins mL/mR/mT/mB wrapped in S() — scale with DPI on 4K
//   v38.6 (May 25, 2025): AutoLRMinDrop 0.1→0.0 — any SOC step-down registers
//   v38.5 (May 25, 2025): 4K auto-scale: FontScale=dpiScale=screenScale=1.5 — fonts AND layout scale together on Dell 4K
//   v38.4 (May 25, 2025): Auto FontScale=2.0 when any screen > 2560px wide (4K detection)
//   v38.3 (May 25, 2025): Scaling locked to 1.0 — identical to v36.6 which worked on all laptops; Dell 4K window is resizable
//   v38.2 (May 25, 2025): DPI detection restored — FontScale AND dpiScale/screenScale set from screen DPI; both fonts and layout scale together
//   v38.1 (May 25, 2025): FontScale hardcoded 1.0 — pixel fonts identical on all displays; DPI detection removed
//   v38.0 (May 25, 2025): All fonts GraphicsUnit.Pixel with pt×96/72×FontScale — prevents GDI double-scaling in per-monitor DPI-aware mode
//   v37.9 (May 25, 2025): Per-monitor DPI aware; FontScale from GetDC(this.Handle) after SetupForm — correct per-monitor DPI, no double-scaling
//   v37.8 (May 25, 2025): FontScale fixed at 1.0; GDI scales pt fonts automatically for System-DPI-aware app — eliminates double-scaling on 150% displays
//   v37.7 (May 25, 2025): Auto-LR left anchor uses regression value at first corner time (a+b*t0), not actual SOC — correct least-squares line
//   v37.6 (May 25, 2025): Auto-LR line starts at first corner point (autoLRTimes[0]/autoLRSOC[0]) not extrapolated to left axis — line visibly passes through the N step-down points
//   v37.5 (May 25, 2025): Auto-LR line properly clipped to plot area — interpolates exit point when SOC=5% target is below visible axis range
//   v37.4 (May 25, 2025): SOC Y2 axis: tight padding around data range (no forced 0 floor); max capped at 100%; tick bounds used for axis limits
//   v37.3 (May 25, 2025): Y2 axis expands to rounded tick bounds — labels no longer crowded at edges; SOC capped at 0–100%
//   v37.2 (May 25, 2025): Reverted to System DPI awareness + GetDeviceCaps manual scaling; removes per-monitor complications on external 4K monitors
//   v37.1 (May 25, 2026): All fonts use standard pt (no GraphicsUnit.Pixel); GDI+ scales for actual monitor DPI automatically — correct size on any display
//   v37.0 (May 25, 2026): FontScale refined from CreateGraphics().DpiX after handle created — gets actual per-monitor DPI on multi-monitor setups
//   v36.9 (May 25, 2026): Auto-LR ETA refreshed every sample (not just on step-down) via RefreshAutoLRETA(); intercept stored as absolute time
//   v36.8 (May 25, 2026): FontScale set from screen DPI at startup (fonts scale with display density; layout stays fixed); fixes tiny fonts on 4K monitors
//   v36.7 (May 24, 2026): X axis "Elapsed Time" label shifted down S(32)→S(42)
//   v36.6 (May 24, 2026): Psys default value 0→4.5 W
//   v36.5 (May 24, 2026): ResetEfficiencyBaseline() called at state transition to Charging — syncs TC66 and Ec baselines to same sample; threshold 0.1→0.01 Wh
//   v36.4 (May 24, 2026): ETA row expands when auto-LR activates (even without TC66); collapses when ETA cleared and TC66 disconnected
//   v36.3 (May 24, 2026): Header font 20pt (26.7px); Absolute row styles S(44)/0; clean expand/collapse on TC66 connect/disconnect
//   v36.2 (May 24, 2026): Header reverted to fixed S(80)/Percent rows — stable two-row layout; TC66 row empty but not collapsed (collapse caused invisible header)
//   v36.1 (May 24, 2026): Header row heights S(30)→S(40) single, S(60)→S(80) double — fits 14pt pixel font with padding
//   v36.0 (May 24, 2026): Header row collapse uses Absolute RowStyle(0) — no gray space when TC66 disconnected; font reverted to 14pt; height S(30)/S(60)
//   v35.9 (May 24, 2026): Header font 14→20pt; tbl stored as headerTbl field; height S(30) single-row when TC66 disconnected, S(60) two-row when connected
//   v35.8 (May 24, 2026): All fonts converted to GraphicsUnit.Pixel (pt×96/72); identical physical size at all DPI settings
//   v35.7 (May 24, 2026): dpiScale/screenScale fixed at 1.0 — per-monitor DPI-aware app; pt fonts and layout scale automatically via OS; eliminates double-scaling at all DPI settings
//   v35.6 (May 24, 2026): Remove dpiScale from ScaleFont() and SF() — per-monitor DPI-aware app; OS scales pt fonts automatically; only S() retains pixel scaling
//   v35.5 (May 24, 2026): Header font uses raw pt (no SF) — fixes double-scaling at 150% DPI; 14pt works correctly at all DPI levels
//   v35.4 (May 24, 2026): Header TableLayoutPanel switched to percent-based columns — fits any window width at any DPI without overflow
//   v35.3 (May 24, 2026): Auto-LR gated on Discharging state in both chart draw and UpdateAutoLR; ClearAutoLR called on transition away from discharge
//   v35.2 (May 24, 2026): Auto-LR line extends to both axes; orange DashDot style (distinct from manual LR pink Dash); clips to right edge if intercept off-screen
//   v35.1 (May 24, 2026): Auto-LR line moved outside manual LR block — now draws independently
//   v35.0 (May 24, 2026): ETA moved to column 9 row 1 (shared with SOC column 9 row 0); 11th column removed
//   v34.9 (May 24, 2026): Auto-LR line start anchored to actual corner SOC value, not regression estimate
//   v34.8 (May 24, 2026): Auto-LR: try-catch guard around UpdateAutoLR; lastAutoSOC reset on state transition; nudLRPoints null guard
//   v34.7 (May 24, 2026): Auto-LR: continuous SOC step-down regression → ETA in header + dashed line in Chart 2; LR pts spinner (3-10, default 5) in footer
//   v34.6 (May 23, 2026): DPI detection uses GetDC(desktop)+GetDeviceCaps — works before form handle exists; fixes 150% DPI on Windows 10
//   v34.5 (May 23, 2026): Header font 22→20pt (100% DPI), 25→23pt (150% DPI) — fits all fields without clipping
//   v34.4 (May 23, 2026): HW() helper measures column widths with actual header font — fixes clipped text in TableLayoutPanel
//   v34.3 (May 23, 2026): TableLayoutPanel column widths now use BW() font metrics — no more clipped text
//   v34.2 (May 23, 2026): Header replaced FlowLayoutPanel with TableLayoutPanel — fixed-width columns give pixel-perfect vertical alignment between BMS and TC66 rows
//   v34.1 (May 23, 2026): DPI fix for Windows 10 — SetProcessDpiAwareness(PerMonitor) called before any graphics context; fixes huge fonts on 150% DPI displays
...
namespace BatteryMonitor
{
    ...
    public class BatteryReading
    {
        ...
        // Extended WMI battery data (cached every 10s)
        public double DirectPower_mW;        // BatteryStatus.DischargeRate/ChargeRate
        public double RemainingCapacity_mWh; // BatteryStatus.RemainingCapacity
        public int    BmsETA_min;            // Win32_Battery.EstimatedRunTime (minutes)
        public int    CycleCount;            // BatteryCycleCount.CycleCount
        public double FullChargedCapacity_mWh; // BatteryFullChargedCapacity
        public double DesignCapacity_mWh;    // BatteryStaticData.DesignedCapacity
        public double HealthPercent;         // FullCharged/Designed × 100
    ...
    public class BatteryReader
    {
        ...
        private bool   cachedHasTemperature = false;
        private double cachedDirectPower_mW = 0;
        private double cachedRemainingCapacity_mWh = 0;
        private int    cachedBmsETA_min = 0;
        private int    cachedCycleCount = 0;
        private double cachedFullChargedCapacity_mWh = 0;
        private double cachedDesignCapacity_mWh = 0;
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (status == 0)
                    {
                        ...
                        // Refresh all WMI data every 10 seconds
                        ...
                        reading.HasTemperature     = cachedHasTemperature;
                        reading.DirectPower_mW     = cachedDirectPower_mW;
                        reading.RemainingCapacity_mWh = cachedRemainingCapacity_mWh;
                        reading.BmsETA_min         = cachedBmsETA_min;
                        reading.CycleCount         = cachedCycleCount;
                        reading.FullChargedCapacity_mWh = cachedFullChargedCapacity_mWh;
                        reading.DesignCapacity_mWh = cachedDesignCapacity_mWh;
                        reading.HealthPercent      = cachedDesignCapacity_mWh > 0
                            ? cachedFullChargedCapacity_mWh / cachedDesignCapacity_mWh * 100.0 : 0;

                        // Use DirectPower (BatteryStatus) if available, else NtPowerInformation.Rate
                        reading.Power_W = cachedDirectPower_mW > 0
                            ? cachedDirectPower_mW / 1000.0
                            : Math.Abs(state.Rate) / 1000.0;

                        if (reading.Voltage_V > 0)
                            reading.Current_mA = reading.Power_W * 1000.0 / reading.Voltage_V;
                        else
        ...
        private void GetAllWMIData()
        {
            // BatteryStatus: voltage, direct power, remaining capacity, state flags
            try
            {
                using (var s = new ManagementObjectSearcher("root\\WMI",
                    "SELECT Voltage,DischargeRate,ChargeRate,RemainingCapacity,Charging,Discharging,PowerOnline FROM BatteryStatus"))
                    foreach (ManagementObject o in s.Get())
                    {
                        object v = o["Voltage"];
                        if (v != null) { double val = Convert.ToDouble(v); if (val > 0) cachedVoltage_mV = val; }
                        object dr = o["DischargeRate"];
                        object cr = o["ChargeRate"];
                        // Check both independently — DischargeRate is non-null but 0 during charging,
                        // which blocked the else-if from ever reading ChargeRate
                        double d = (dr != null) ? Convert.ToDouble(dr) : 0;
                        double c = (cr != null) ? Convert.ToDouble(cr) : 0;
                        if (d > 0) cachedDirectPower_mW = d;
                        else if (c > 0) cachedDirectPower_mW = c;
                        else cachedDirectPower_mW = 0;  // both zero = idle; falls back to NtPowerInformation
                        object rc = o["RemainingCapacity"];
                        if (rc != null) cachedRemainingCapacity_mWh = Convert.ToDouble(rc);
                    }
            }
            catch { }

            // MSAcpi_ThermalZoneTemperature: battery temperature
            // Strategy: prefer zone whose InstanceName contains "BAT"; otherwise take minimum
            try
            {
                using (var s = new ManagementObjectSearcher("root\\WMI",
                    "SELECT InstanceName, CurrentTemperature FROM MSAcpi_ThermalZoneTemperature"))
                {
                    double batTemp = double.MaxValue;
                    ...
                    foreach (ManagementObject o in s.Get())
                    {
                        ...
                        if (t == null) continue;
                        double c = (Convert.ToDouble(t) - 2732.0) / 10.0;
                        if (c < -20 || c > 80) continue;
                        if (c < minTemp) minTemp = c;
                        if (name.ToUpper().Contains("BAT") && c < batTemp) { batTemp = c; hasBat = true; }
                    }
                    double chosen = hasBat ? batTemp : minTemp;
                    if (chosen < double.MaxValue) { cachedTemperature_C = chosen; cachedHasTemperature = true; }
                }
            }
            catch { }

            // Win32_Battery: BMS ETA in minutes
            try
            {
                using (var s = new ManagementObjectSearcher("root\\CIMV2",
                    ...
                    foreach (ManagementObject o in s.Get())
                    {
                        object eta = o["EstimatedRunTime"];
                        if (eta != null) { uint val = Convert.ToUInt32(eta); if (val > 0 && val < 65535) cachedBmsETA_min = (int)val; }
                    }
            }
            catch { }

            // BatteryCycleCount: charge cycles
            try
            {
                using (var s = new ManagementObjectSearcher("root\\WMI",
                    ...
                    foreach (ManagementObject o in s.Get())
                    {
                        object c = o["CycleCount"];
                        if (c != null) { uint val = Convert.ToUInt32(c); if (val > 0) cachedCycleCount = (int)val; }
                    }
            }
            catch { }

            // BatteryFullChargedCapacity + BatteryStaticData: health
            try
            {
                using (var s = new ManagementObjectSearcher("root\\WMI",
                    ...
                    foreach (ManagementObject o in s.Get())
                    {
                        object fc = o["FullChargedCapacity"];
                        if (fc != null) { double val = Convert.ToDouble(fc); if (val > 0) cachedFullChargedCapacity_mWh = val; }
                    }
            }
            catch { }
            ...
            try
            {
                using (var s = new ManagementObjectSearcher("root\\WMI",
                    ...
                    foreach (ManagementObject o in s.Get())
                    {
                        object dc = o["DesignedCapacity"];
                        if (dc != null) { double val = Convert.ToDouble(dc); if (val > 0) cachedDesignCapacity_mWh = val; }
                    }
            }
            catch { }
        }
    ...
    public static class TickCalc
    {
        ...
        // 3 significant figures: XXX / XX.X / X.XX / 0.XXX
        public static string Fmt3(double val)
        {
            double abs = Math.Abs(val);
            if (abs >= 100.0) return val.ToString("F0");
            if (abs >= 10.0)  return val.ToString("F1");
            if (abs >= 1.0)   return val.ToString("F2");
    ...
    public class DualAxisChartPanel : Panel
    {
        ...
        public float DpiScale = 1.0f;  // Fixed — per-monitor DPI-aware; pt fonts auto-scale
        public float FontScale = 1.0f;  // Global font scale multiplier (set by parent form)
        public int MarginLeft  = 135;   // Left plot margin (scaled units) — override per chart
        public int MarginRight = 110;   // Right plot margin (scaled units) — override per chart
        
        // Helper method to scale font sizes based on DPI
        private float ScaleFont(float baseSize)
        {
            return baseSize * 96.0f / 72.0f * FontScale;  // pt→px at 96dpi × FontScale
        }
        ...
        public int MaxTicks = 8;  // 5-7 ticks visible including extremes
        ...
        // Auto-LR fields (set by MainForm)
        private List<double> autoLRTimes = null;
        private List<double> autoLRSOC   = null;
        private double autoLR_a = 0, autoLR_b = 0, autoLR_r2 = 0;
        private double autoLRInterceptTime = 0;  // elapsed sec when SOC=5%
        private bool autoLRValid = false;

        public void SetAutoLR(List<double> times, List<double> soc,
            double a, double b, double r2, double interceptTime)
        {
            autoLRTimes = new List<double>(times);
            autoLRSOC   = new List<double>(soc);
            autoLR_a = a; autoLR_b = b; autoLR_r2 = r2;
            autoLRInterceptTime = interceptTime;
            autoLRValid = true;
            this.Invalidate();
        }

        public void ClearAutoLR()
        {
            autoLRValid = false;
            autoLRTimes = null; autoLRSOC = null;
            this.Invalidate();
        }
        ...
        public Color TempColor         = Color.Red;  // TC66 temp — red line
        ...
        private void ComputeAvg()
        {
            ...
            avgValue = sum / count;
            avgResult = name + " avg = " + Fmt3(avgValue) + " " + unit;
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            int mL = S(MarginLeft), mR = S(MarginRight), mT = S(48), mB = S(95);  // Scale margins with DPI
            ...
            using (var font = new Font("Segoe UI", ScaleFont(23f), FontStyle.Regular, GraphicsUnit.Pixel))
            ...
            for (int i = 0; i < xDataTemp.Count; i++)
            {
                ...
                {
                    // TC66 temp not plotted — excluded from Y2 range
                }
            }
            
            // Include TC66 temperature data in Y2 min/max
            for (int i = 0; i < xDataTemp.Count; i++)
            {
                if (xDataTemp[i] >= xFirst)
                {
            ...
            if (y2Range > 0)
            {
                double y2Pad = (Y2Unit == "%") ? 3.0 :
                               (Y2Unit == "A" || Y2Unit == "Ah") ? 0.5 : 2.0;
                if (Y2Unit == "%")
                {
                    // SOC: tight padding; only add 10% headroom when data is near 100%
                    visY2Min -= y2Pad;
                    if (visY2Max >= 90.0)
                        visY2Max = 110.0;  // Headroom above 100% only when near full charge
                    else
                        visY2Max += y2Pad;  // Normal tight padding otherwise
                }
                else if (!y2DataHasNegative) visY2Min = 0;
                else visY2Min -= y2Pad;
                if (Y2Unit != "%") visY2Max += y2Pad;
            }
            // SOC ceiling already set in padding block above
            ...
            // Y2 ticks: expand axis to rounded tick bounds so labels never crowd edges
            double y2tMin, y2tMax, y2tStep;
            TickCalc.GetNiceTicks(visY2Min, visY2Max, MaxTicks, out y2tMin, out y2tMax, out y2tStep);
            // Use rounded tick bounds as axis limits — gives clean labels with breathing room
            double y2AxisMin = y2tMin;
            double y2AxisMax = y2tMax;
            if (y2AxisMin < 0 && !y2DataHasNegative) y2AxisMin = 0;
            if (Y2Unit == "%")
            {
                // SOC: fixed ceiling 110 (10% breathing room above 100%)
                // No label drawn above 100 — ticks only up to 100
                // Cap at 110 only if data was near 100% (ceiling set above)
                y2AxisMax = Math.Min(y2tMax, visY2Max);
                y2AxisMin = Math.Max(y2tMin, 0);
            }
            ...
            // X axis title
            using (var font = new Font("Segoe UI", ScaleFont(22f), FontStyle.Regular, GraphicsUnit.Pixel))  // Axis title
            using (var brush = new SolidBrush(TextColor))
            {
                ...
                g.DrawString(xTitle, font, brush,
                    pa.Left + (pa.Width - sz.Width) / 2, pa.Bottom + S(42));  // Clear space below tick labels (+half line)
            }
            ...
            Color capChargeColor = Color.FromArgb(0x00, 0xBC, 0xD4);  // Teal (Qc)
            Color capTotalColor  = Color.FromArgb(0x00, 0x83, 0x8F);  // Darker teal (Qt)
            ...
            if (EnableLR && lrClickState >= 1)
            {
                ...
                if (lrClickState == 2)
                {
                    ...
                    {
                        using (var font = new Font("Segoe UI", ScaleFont(20f), FontStyle.Bold, GraphicsUnit.Pixel))
                        using (var brush = new SolidBrush(LRLineColor))
                        {
                            ...
                            if (RunTimeSeconds > 0)
                            {
                                ...
                                using (var timeFont = new Font("Segoe UI", ScaleFont(20f), FontStyle.Bold, GraphicsUnit.Pixel))
            ...
            // --- Auto-LR dashed line (independent of manual LR) ---
            if (autoLRValid && autoLRTimes != null && autoLRTimes.Count >= 3
                && CurrentBatteryState == BatteryState.Discharging)
            {
                // Extend line from left axis to right edge (or SOC=5% intercept, whichever comes first)
                double alrYRight = autoLR_a + autoLR_b * (xFirst + xRange);  // at right edge
                double alrXEnd   = autoLRInterceptTime;
                double alrYEnd   = 5.0;

                // Left anchor: always extrapolate regression to left axis edge
                double alrYLeft = autoLR_a + autoLR_b * xFirst;
                double alrPxLf = pa.Left;
                double alrPyLf = pa.Bottom - (alrYLeft - y2AxisMin) / y2Span * pa.Height;
                // Right endpoint: whichever comes first — intercept or right edge
                double alrPxRf, alrPyRf;
                if (alrXEnd <= xFirst + xRange)
                {
                    alrPxRf = pa.Left + (alrXEnd - xFirst) / xRange * pa.Width;
                    alrPyRf = pa.Bottom - (alrYEnd   - y2AxisMin) / y2Span * pa.Height;
                }
                else
                {
                    alrPxRf = pa.Right;
                    alrPyRf = pa.Bottom - (alrYRight - y2AxisMin) / y2Span * pa.Height;
                }
                // Clip line to plot area bounds (prevent drawing outside chart)
                // If right endpoint is below plot bottom, interpolate where line crosses bottom
                if (alrPyRf > pa.Bottom && alrPxRf > alrPxLf)
                {
                    double t = (pa.Bottom - alrPyLf) / (alrPyRf - alrPyLf);
                    alrPxRf = alrPxLf + t * (alrPxRf - alrPxLf);
                    alrPyRf = pa.Bottom;
                }
                if (alrPyLf < pa.Top && alrPxRf > alrPxLf)
                {
                    double t = (pa.Top - alrPyLf) / (alrPyRf - alrPyLf);
                    alrPxLf = alrPxLf + t * (alrPxRf - alrPxLf);
                    alrPyLf = pa.Top;
                }
                int alrPxL = (int)alrPxLf, alrPyL = (int)alrPyLf;
                int alrPxR = (int)alrPxRf, alrPyR = (int)alrPyRf;
                // Skip if line is entirely outside plot area
                if (alrPyL > pa.Bottom && alrPyR > pa.Bottom) goto skipAutoLR;

                Color autoLRCol = autoLR_r2 >= 0.9
                    ? Color.FromArgb(255, 140, 0)   // Orange — distinct from manual LR pink
                    : Color.FromArgb(180, 160, 120); // Muted amber if unreliable
                using (var pen = new Pen(autoLRCol, 1.5f)
                    { DashStyle = System.Drawing.Drawing2D.DashStyle.DashDot })
                    g.DrawLine(pen, alrPxL, alrPyL, alrPxR, alrPyR);

                // Small circle at endpoint
                using (var pen = new Pen(autoLRCol, 1.5f))
                    g.DrawEllipse(pen, alrPxR - 4, alrPyR - 4, 8, 8);
                skipAutoLR:;
            }
            ...
            if (yData1.Count > 0)
            {
                ...
                string fmt = ChartTitle.Contains("Voltage") ? "F2" : null;
                ...
                if (ChartTitle.Contains("Voltage") && xDataCapCharge.Count > 0)
                {
                    ...
                    Color qcColor = Color.FromArgb(0x00, 0xBC, 0xD4);  // Teal
                    DrawValueBadgeAtY(g, pa, qcVal, "Ah", qcColor, false, null, 38);
                }
                ...
                if (ChartTitle.Contains("Power") && xDataEnergySegment.Count > 0)
                {
                    ...
                    DrawValueBadgeAtY(g, pa, ecVal, "Wh", ecColor, true, null, 38);
                }
            }
            if (yData2.Count > 0)
            {
                ...
                // SOC (%) uses F1; current (A) and power (W) use Fmt3
                string fmt = Y2Unit == "%" ? "F1" : null;
                DrawValueBadge(g, pa, lastVal, Y2Unit, c, false, fmt);

                // Draw TC66 temperature toast in lower right corner
                if (yDataTemp.Count > 0)
                {
                    double tempVal = yDataTemp[yDataTemp.Count - 1];
                    using (var font = new Font("Segoe UI", ScaleFont(23f), FontStyle.Bold, GraphicsUnit.Pixel))
            ...
            if (EnableLR && lrClickState < 2)
            {
                ...
                using (var font = new Font("Segoe UI", ScaleFont(17f), FontStyle.Regular, GraphicsUnit.Pixel))
        ...
        // Reduces xd/yd to at most maxPts points while preserving visual shape.
        // Returns a list of indices into the original arrays.
        private const int LttbMaxPoints = 10000;

        private static List<int> LttbDecimate(List<double> xd, List<double> yd, int maxPts)
        {
            int n = xd.Count;
            var result = new List<int>(maxPts);
            if (n <= maxPts) { for (int i = 0; i < n; i++) result.Add(i); return result; }

            result.Add(0);  // Always keep first point
            double bucketSize = (double)(n - 2) / (maxPts - 2);

            int prevIdx = 0;
            for (int b = 0; b < maxPts - 2; b++)
            {
                // Next bucket range
                int nextStart = (int)((b + 1) * bucketSize) + 1;
                int nextEnd   = Math.Min((int)((b + 2) * bucketSize) + 1, n - 1);

                // Average of next bucket (used as C point)
                double avgX = 0, avgY = 0;
                int avgCount = nextEnd - nextStart;
                for (int i = nextStart; i < nextEnd; i++) { avgX += xd[i]; avgY += yd[i]; }
                avgX /= avgCount; avgY /= avgCount;

                // Current bucket range
                int curStart = (int)(b * bucketSize) + 1;
                int curEnd   = nextStart;

                // Find point in current bucket that forms largest triangle with prevIdx and avg
                double maxArea = -1;
                int maxIdx = curStart;
                double ax = xd[prevIdx], ay = yd[prevIdx];
                for (int i = curStart; i < curEnd; i++)
                {
                    double area = Math.Abs((ax - avgX) * (yd[i] - ay) - (ax - xd[i]) * (avgY - ay));
                    if (area > maxArea) { maxArea = area; maxIdx = i; }
                }
                result.Add(maxIdx);
                prevIdx = maxIdx;
            }
            result.Add(n - 1);  // Always keep last point
            return result;
        }
            ...
            Color chargeCol, Color dischargeCol, Color idleCol, float lineW)
        {
            ...
            // LTTB decimation: cap rendered points to preserve UI responsiveness
            List<int> idx = LttbDecimate(xd, yd, LttbMaxPoints);
            int idxCount = idx.Count;

            int startPos = 0;
            for (int p = 0; p < idxCount; p++)
            {
                if (xd[idx[p]] >= xFirst) { startPos = Math.Max(0, p - 1); break; }
            }

            for (int p = startPos + 1; p < idxCount; p++)
            {
                int i  = idx[p];
                int i1 = idx[p - 1];

                if (sd[i] != sd[i1]) continue;

                double timeGap = xd[i] - xd[i1];
                double expectedInterval = p > 1 ? (xd[i1] - xd[idx[p - 2]]) : timeGap;
                double maxInterval = Math.Max(expectedInterval * 3.0, 3.0);
                if (timeGap > maxInterval) continue;

                int px1 = pa.Left + (int)((xd[i1] - xFirst) / xRange * pa.Width);
                int py1 = pa.Bottom - (int)((yd[i1] - yLow) / ySpan * pa.Height);
        ...
        {
            using (var font = new Font("Segoe UI", ScaleFont(20f), FontStyle.Regular, GraphicsUnit.Pixel))  // Axis titles
            ...
            double y2Low, double y2Span)
        {
            ...
            {
                Color c = Color.FromArgb(0x00, 0xBC, 0xD4);
            ...
            // TC66 Temperature (Y2) → arrow points RIGHT
            if (xDataTemp.Count > 0)
            {
                ...
                {
                    int cx = pa.Left + (int)(xRange > 0 ? (xTarget - xFirst) / xRange * pa.Width : pa.Width / 2);
                    drawArrow(cx, py, false, TempColor);
                }
            }
            ...
            {
                Color c = Color.FromArgb(0x00, 0x83, 0x8F);
            ...
            double y2Min, double y2Span)
        {
            using (var font = new Font("Segoe UI", ScaleFont(17f), FontStyle.Regular, GraphicsUnit.Pixel))
            {
                ...
                // Legend order: SOC first, then Power, then energy/cap, then TC66 temp
                ...
                if (xDataTC66_2.Count > 0)
                {
                    ...
                    string tc66y2Label = y2Legend.Contains(",") ? y2Legend.Replace(",", ".TC,") : y2Legend + ".TC";
                    entries.Add(new LegendEntry(tc66y2Label, tc66y2Col, 0.5f, true));
                }
                ...
                if (xDataTC66_1.Count > 0)
                {
                    ...
                    string tc66y1Label = y1Legend.Contains(",") ? y1Legend.Replace(",", ".TC,") : y1Legend + ".TC";
                    entries.Add(new LegendEntry(tc66y1Label, tc66y1Col, 0.5f, true));
                }
                
                // Add Ec curve (on Y1 axis) - natural sign
                Color segmentEnergyColor = Color.FromArgb(0xBA, 0x68, 0xC8); // Bright medium purple
                if (xDataEnergySegment.Count > 0)
                    entries.Add(new LegendEntry("Energy, Wh", segmentEnergyColor, 3.5f, true));

                // Add Et curve (on Y1 axis) - only when shown from known SOC start
                Color totalEnergyColor = Color.FromArgb(0x8E, 0x24, 0xAA);  // Vivid deep purple
                if (xDataEnergyTotal.Count > 0)
                    entries.Add(new LegendEntry("Energy, Wh", totalEnergyColor, 2.5f, false));

                // Add Qc curve (on Y2 axis, V-i chart only) - natural sign
                Color capChargeColor = Color.FromArgb(0x00, 0xBC, 0xD4);  // Teal (Qc)
                if (xDataCapCharge.Count > 0)
                    entries.Add(new LegendEntry("Capacity, Ah", capChargeColor, 3.5f, false));

                // Add Qt curve (on Y2 axis) - only when shown from known SOC start
                Color capTotalColor = Color.FromArgb(0x00, 0x83, 0x8F);  // Darker teal (Qt)
                if (xDataCapDischarge.Count > 0)
                    entries.Add(new LegendEntry("Capacity, Ah", capTotalColor, 2.5f, false));

                // TC66 Temperature
                if (xDataTemp.Count > 0)
                    entries.Add(new LegendEntry("Temperature, °C", TempColor, 0.5f, true));
                ...
                int totalPts = xData1.Count + xData2.Count + xDataTC66_1.Count + xDataTC66_2.Count
                             + xDataTemp.Count + xDataEnergySegment.Count
        ...
        private void DrawValueBadge(Graphics g, Rectangle pa,
            double val, string unit, Color color, bool leftSide, string format = null)
        {
            using (var font = new Font("Segoe UI", ScaleFont(23f), FontStyle.Bold, GraphicsUnit.Pixel))
            using (var brush = new SolidBrush(color))
            {
                string txt = (format != null ? val.ToString(format) : Fmt3(val)) + " " + unit;
                SizeF sz = g.MeasureString(txt, font);
            ...
            double y2Low, double y2Span)
        {
            ...
            int bestIdxTC66_1 = -1, bestIdxTC66_2 = -1, bestIdxTemp = -1;
            ...
            {
                lines.Add(Y1Title + ": " + (Y1Unit == "V" ? yData1[bestIdx1].ToString("F2") : Fmt3(yData1[bestIdx1])) + " " + Y1Unit);
                curveColor = y1Col;
            }
            ...
            {
                lines.Add("TC66 " + Y1Title + ": " + (Y1Unit == "V" ? yDataTC66_1[bestIdxTC66_1].ToString("F2") : Fmt3(yDataTC66_1[bestIdxTC66_1])) + " " + Y1Unit);
            ...
            {
                lines.Add("Ec (Wh): " + Fmt3(yDataEnergySegment[bestIdxEnergySegment]));
                curveColor = Color.FromArgb(0xBA, 0x68, 0xC8);
            }
            else if (closestCurve == "EnergyTotal" && bestIdxEnergyTotal >= 0)
            {
                lines.Add("Et (Wh): " + Fmt3(yDataEnergyTotal[bestIdxEnergyTotal]));
                curveColor = Color.FromArgb(0x8E, 0x24, 0xAA);
            }
            else if (closestCurve == "CapCharge" && bestIdxCapCharge >= 0)
            {
                lines.Add("Qc (Ah): " + Fmt3(yDataCapCharge[bestIdxCapCharge]));
                curveColor = Color.FromArgb(0x00, 0xBC, 0xD4);
            }
            else if (closestCurve == "CapDischarge" && bestIdxCapDischarge >= 0)
            {
                lines.Add("Qt (Ah): " + Fmt3(yDataCapDischarge[bestIdxCapDischarge]));
                curveColor = Color.FromArgb(0x00, 0x83, 0x8F);
            }

            // Draw tooltip box
            using (var font = new Font("Segoe UI", ScaleFont(16f), FontStyle.Bold, GraphicsUnit.Pixel))
            {
                float maxWidth = 0;
                float totalHeight = 0;
    ...
    {
        private const string AppVersion = "v46.0";  // ← UPDATE ONLY HERE
        ...
        private static float FontScale = 1.0f;      // Set from DPI at startup; ← or adjust manually
        ...
        private TableLayoutPanel headerTbl;  // Header table — height toggled with TC66 row  // True when viewing a loaded CSV — η is meaningless then

        // Auto-LR: continuous discharge time prediction from SOC step-down corners
        private NumericUpDown nudLRPoints;
        private Label lblAutoETA;
        private readonly List<double> autoLRTimes = new List<double>();   // elapsed seconds at each step
        private readonly List<double> autoLRSOC   = new List<double>();   // SOC value at each step
        private double lastAutoSOC = double.NaN;  // SOC at previous sample for step detection
        private const double AutoLRFloor = 5.0;   // Predict to 5% SOC
        private const double AutoLRMinDrop = 0.0; // Any SOC decrease registers as a step
        private double mfAutoLR_b = 0;           // Slope — MainForm copy for RefreshAutoLRETA
        private double mfAutoLR_r2 = 0;          // R² — MainForm copy
        private double mfAutoLRIntercept = 0;    // Absolute intercept time — MainForm copy
        private bool   mfAutoLRValid = false;    // Valid fit exists
        ...
        private BatteryState prevRawState  = BatteryState.Idle;  // Raw previous state for charge-complete detection
        // ── Alert fields ──
        private System.Threading.Timer beepTimer;   // Fires every 60s for charge-complete alert
        private readonly SpeechSynthesizer _synth = new SpeechSynthesizer();
        private int  beepFireCount  = 0;            // Counts 60s firings; stops at 10
        private int  lastSOCBeep    = 999;          // Last SOC integer at which low-SOC beep fired
        private Button btnStopBeep;                 // Red dismiss button for charge-complete alert
        private Panel  ctrlBar;                     // Footer control bar (needed by alert methods)

        // Header labels: live readings + segment and total capacity/energy
        private Label lblStatus, lblVoltage, lblCurrent, lblResistance, lblPower, lblSOC, lblBMSTemp, lblHealth, lblCycles;
        ...
        public MainForm()
        {
            dpiScale = 1.0f; screenScale = 1.0; FontScale = 1.0f;
            // Auto-scale fonts AND layout on 4K displays (physical width > 2560px)
            if (Screen.PrimaryScreen.Bounds.Width > 2560 ||
                Screen.AllScreens.Any(s => s.Bounds.Width > 2560))
            {
                FontScale = 1.5f;   // 1.5× fonts on 4K
                dpiScale = 1.5f;    // 1.5× layout (S() values)
                screenScale = 1.5;  // same
            }
            SetupForm();
            BuildUI();
            SetupTimers();
            ...
            this.Resize += (s, e) => { chartVI.Invalidate(); chartPS.Invalidate(); AdjustHeaderFontSize(); };
        ...
        {
            return (float)(baseValue * 96.0f / 72.0f * FontScale);  // pt→px at 96dpi × FontScale
        }
        ...
        private Font _headerFontRef;  // Header font ref for column width measurement
        private int  headerFontSize = 20;  // Current header font pt; auto-shrinks to fit (min 14)
        private int HW(string text, int extraPad = 10)
        {
            Font f = _headerFontRef ?? SystemFonts.DefaultFont;
            return TextRenderer.MeasureText(text, f).Width + extraPad;
        }

        private void BuildUI()
        {
            // ---- HEADER: two-row TableLayoutPanel for pixel-perfect column alignment ----
            // Column widths (S-scaled) sized for widest possible value in each field.
            // BMS row:  Status | V=    | i=    | R=    | P=    | Qc=   | Ec=   | Qt=   | Et=   | SOC=
            // TC66 row: TC66   | V=    | i=    | R=    | P=    | Qin=  | Ein=  | η=    | T=
            Color TC66Col = Color.FromArgb(0, 120, 60);

            lblStatus     = MakeHeaderLabel("IDLE",         GrayCol);
            lblVoltage    = MakeHeaderLabel("U=-- V",       TextCol);
            ...
            lblSOC.BackColor = Color.FromArgb(255, 255, 180);
            lblBMSTemp    = MakeHeaderLabel("",              Color.Red);  // TC66 temp — red
            lblBMSTemp.Visible = false;
            lblHealth     = MakeHeaderLabel("SoH=--%",       Color.FromArgb(0, 100, 60));
            lblCycles     = MakeHeaderLabel("Cyc=--",        Color.FromArgb(0, 100, 60));
            lblSegCap     = MakeHeaderLabel("Qc=0.00 Ah",   Color.FromArgb(0x00, 0xBC, 0xD4));  // Teal (matches Qc curve)
            lblSegEnergy  = MakeHeaderLabel("Ec=0.00 Wh",   Color.FromArgb(0xBA, 0x68, 0xC8));  // Medium purple (matches Ec curve)
            lblTotCap     = MakeHeaderLabel("Qt=0.00 Ah",   Color.FromArgb(0x00, 0x83, 0x8F));  // Darker teal (matches Qt curve)
            lblTotEnergy  = MakeHeaderLabel("Et=0.00 Wh",   Color.FromArgb(0x8E, 0x24, 0xAA));  // Deep purple (matches Et curve)

            lblTC66Status     = MakeHeaderLabel("TC66",          TC66Col);
            lblTC66Volt       = MakeHeaderLabel("U=-- V",        TC66Col);
            ...
            lblTC66Temp       = MakeHeaderLabel("",              TC66Col);
            lblAutoETA        = MakeHeaderLabel("",              Color.FromArgb(180,0,180));
            lblAutoETA.BackColor = Color.FromArgb(255, 255, 180);  // Yellow like SOC
            lblAutoETA.Visible = false;
            lblTC66mAh        = MakeHeaderLabel("Qin=0.00 Ah",   TC66Col);
            lblTC66mWh        = MakeHeaderLabel("Ein=0.00 Wh",   TC66Col);
            lblTC66Eff        = MakeHeaderLabel("CE=--%",        Color.FromArgb(180, 0, 180));

            // Column widths: sized for widest realistic value (negative, Ω, long labels)
            // Header font size
            int hdrFontSize = (dpiScale <= 1.0f) ? 20 : 23;

            // Use percent-based columns so header always fits window at any DPI
            // Weights reflect relative text widths of widest realistic values
            float[] colPct = new float[] {
                6f,   // 0: Status  "DCHRG"
                9f,   // 1: V=      "-19.99 V"
                8f,   // 2: i=      "-3.00 A"
                7f,   // 3: R=      "999.9 Ω"
                8f,   // 4: P=      "-99.9 W"
                10f,  // 5: Qc/Qin  "Qin=99.99 Ah"
                11f,  // 6: Ec/Ein  "Ein=999.99 Wh"
                10f,  // 7: Qt/CE   "CE=68.5%"
                10f,  // 8: Et/T.TC  "T.TC=43°C"
                12f,  // 9: SOC/ETA
                9f,   // 10: SoH=/Cyc= "SoH=97.2%"
            };

            headerTbl = new TableLayoutPanel
            {
                RowCount = 2, ColumnCount = 11,
                Dock = DockStyle.Top,
                Height = S(44),
                BackColor = PanelBg,
            ...
            headerTbl.RowStyles.Add(new RowStyle(SizeType.Absolute, S(44)));  // BMS row
            headerTbl.RowStyles.Add(new RowStyle(SizeType.Absolute, 0));       // TC66 row (collapsed)
            for (int c = 0; c < 11; c++)
                headerTbl.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, colPct[c]));

            // Row 0: BMS
            headerTbl.Controls.Add(lblStatus,     0, 0);
            headerTbl.Controls.Add(lblVoltage,    1, 0);
            headerTbl.Controls.Add(lblCurrent,    2, 0);
            headerTbl.Controls.Add(lblResistance, 3, 0);
            headerTbl.Controls.Add(lblPower,      4, 0);
            headerTbl.Controls.Add(lblSegCap,     5, 0);
            headerTbl.Controls.Add(lblSegEnergy,  6, 0);
            headerTbl.Controls.Add(lblTotCap,     7, 0);
            headerTbl.Controls.Add(lblTotEnergy,  8, 0);
            headerTbl.Controls.Add(lblSOC,        9, 0);
            headerTbl.Controls.Add(lblAutoETA,    9, 1);
            headerTbl.Controls.Add(lblBMSTemp,    8, 1);  // T.TC= label — col 8 row 1
            headerTbl.Controls.Add(lblHealth,     10, 0); // Battery health %
            headerTbl.Controls.Add(lblCycles,     10, 1); // Charge cycles

            // Row 1: TC66
            headerTbl.Controls.Add(lblTC66Status,     0, 1);
            headerTbl.Controls.Add(lblTC66Volt,       1, 1);
            headerTbl.Controls.Add(lblTC66Amp,        2, 1);
            headerTbl.Controls.Add(lblTC66Resistance, 3, 1);
            headerTbl.Controls.Add(lblTC66Power,      4, 1);
            headerTbl.Controls.Add(lblTC66mAh,        5, 1);
            headerTbl.Controls.Add(lblTC66mWh,        6, 1);
            headerTbl.Controls.Add(lblTC66Eff,        7, 1);
            // lblTC66Temp removed from table; BMSTemp owns col 8,1

            // Set TC66 row background
            foreach (var lbl in new[] { lblTC66Status, lblTC66Volt, lblTC66Amp, lblTC66Resistance,
                ...
                lbl.BackColor = Color.FromArgb(220, 235, 220);

            // TC66 row initially hidden; collapse to single row height
            foreach (Control c in new Control[] { lblTC66Status, lblTC66Volt, lblTC66Amp,
                lblTC66Resistance, lblTC66Power, lblTC66mAh, lblTC66mWh, lblTC66Eff, lblTC66Temp })
                c.Visible = false;

            // tc66HeaderPanel kept for Visible toggle — point it at tbl row 1 via Tag
            tc66HeaderPanel = new Panel { Visible = false };  // Dummy — visibility managed per-label
            ...
            Font controlFont = new Font("Segoe UI", SF(14), FontStyle.Bold, GraphicsUnit.Pixel);
            _controlFontRef = controlFont;  // Make available to BW() helper
            
            ctrlBar = new Panel { Dock = DockStyle.Bottom, Height = barH, BackColor = PanelBg };
            ...
            btnTC66ScreenFlip = new Button
            {
                ...
                Font = new Font("Segoe UI", SF(20), FontStyle.Bold, GraphicsUnit.Pixel),
            ...
            Label llrp = new Label
            {
                Text = "LR pts:", ForeColor = Color.FromArgb(180, 0, 180), AutoSize = true,
                Font = controlFont, TextAlign = ContentAlignment.MiddleLeft
            };
            nudLRPoints = new NumericUpDown
            {
                Minimum = 3, Maximum = 10, Value = 5, DecimalPlaces = 0, Increment = 1,
                Width = S(45), Height = btnHeight, Font = controlFont,
            ...
            {
                Minimum = 0, Maximum = 100, Value = 4.5M, DecimalPlaces = 1, Increment = 0.5M,
            ...
            lblElapsed = new Label
            {
                ...
                Font = new Font("Consolas", SF(19), FontStyle.Bold, GraphicsUnit.Pixel),
                TextAlign = ContentAlignment.MiddleLeft,
                Margin = new Padding(0)
            };
            
            btnStopBeep = new Button
            {
                Text = "\U0001F507", Width = BW("\u2715 Beep"), Height = btnHeight,
                Font = controlFont, ForeColor = Color.White, BackColor = Color.Red,
                FlatStyle = FlatStyle.Flat, Margin = new Padding(S(3)),
                Visible = false
            };
            btnStopBeep.FlatAppearance.BorderColor = Color.DarkRed;
            btnStopBeep.Click += (s, e) => StopChargeCompleteAlert();
            ...
            Control[] ctrls = new Control[] {
                ...
                li, nudInterval, ls,
                btnExportCSV, lw, cmbTimeWindow, btnTC66Refresh,
                ltc, cmbTC66Port, btnTC66Connect, btnTC66ScreenFlip,
                lps, nudPsys, lpsw,
                llrp, nudLRPoints,
                lrt, lblElapsed, lblSamples, btnStopBeep
            };
            int[] gaps = new int[] {
                4, 4, 8,
                2, 2, 8,
            ...
            ctrlBar.Layout += (s, ev) => {
                ...
                // Right-anchor the stop-beep button
                if (btnStopBeep != null && btnStopBeep.Visible)
                {
                    btnStopBeep.Left = ctrlBar.Width - btnStopBeep.Width - S(8);
                    btnStopBeep.Top  = (h - btnStopBeep.Height) / 2;
                }
            };
            ...
            chartVI = new DualAxisChartPanel
            {
                ...
                Y2ChargeColor = Color.FromArgb(0x00, 0xAA, 0x00),      // Bright green (BMS current charge)
                Y2DischargeColor = Color.FromArgb(0xFF, 0x00, 0x00),   // Pure red (matches Temperature curve)
                ...
                TC66Y2ChargeColor = Color.FromArgb(0x50, 0xC0, 0x50),     // Light bright green (TC66 current charge)
            ...
            chartPS = new DualAxisChartPanel
            {
                ...
                Y2ChargeColor = Color.FromArgb(0x1B, 0x5E, 0x20),
                Y2DischargeColor = Color.FromArgb(0xFF, 0x00, 0x00),  // Pure red (matches T.TC display)
                ...
                TC66Y2ChargeColor = Color.FromArgb(0x55, 0x8B, 0x2F),
            ...
            // Dock order: add bottom-to-top (last added = topmost)
            this.Controls.Add(chartGrid);
            this.Controls.Add(ctrlBar);
            this.Controls.Add(headerTbl);   // Two-row header table (BMS + TC66)
        }
        ...
        private Label MakeHeaderLabel(string text, Color color)
        {
            ...
            // 14pt renders correctly at both 100% and 150% DPI when not double-scaled
            return MakeHeaderLabel(text, color, 20);
        }

        private Label MakeHeaderLabel(string text, Color color, int fontSize = 16)
        {
            // fontSize in raw pt — DPI-aware app; Windows scales pt automatically.
            // Do NOT use SF() here: that would double-scale at 150% DPI.
            return new Label
            {
                Text = text, ForeColor = color,
                Font = new Font("Segoe UI", fontSize * 96f / 72f * FontScale, FontStyle.Bold, GraphicsUnit.Pixel),
                AutoSize = true,
                Margin = Padding.Empty,
        ...
        // Auto-shrink header fonts until all labels fit their columns (min 14pt)
        private void AdjustHeaderFontSize()
        {
            if (headerTbl == null || headerTbl.Width == 0) return;
            var allLabels = new List<Label>();
            foreach (Control c in headerTbl.Controls)
                if (c is Label) allLabels.Add((Label)c);
            if (allLabels.Count == 0) return;

            // Try from max size down; apply font, compare PreferredWidth vs column pixel width
            for (int sz = 20; sz >= 14; sz--)
            {
                float px = sz * 96f / 72f * FontScale;
                var font = new Font("Segoe UI", px, FontStyle.Bold, GraphicsUnit.Pixel);
                foreach (Label lbl in allLabels) lbl.Font = font;
                bool fits = true;
                foreach (Label lbl in allLabels)
                {
                    if (!lbl.Visible || lbl.Text.Length == 0) continue;
                    int col = headerTbl.GetColumn(lbl);
                    if (col < 0 || col >= headerTbl.ColumnStyles.Count) continue;
                    float colW = headerTbl.Width * headerTbl.ColumnStyles[col].Width / 100f;
                    if (lbl.PreferredWidth + lbl.Margin.Horizontal > colW) { fits = false; break; }
                }
                if (fits) { headerFontSize = sz; return; }
                if (sz > 14) font.Dispose();
            }
            headerFontSize = 14;
        }
        ...
        private void OnTC66Connect(object sender, EventArgs e)
        {
            ...
            {
                // Show TC66 row and expand header
                foreach (Control c in new Control[] { lblTC66Status, lblTC66Volt, lblTC66Amp,
                    lblTC66Resistance, lblTC66Power, lblTC66mAh, lblTC66mWh, lblTC66Eff })
                    c.Visible = true;
                headerTbl.RowStyles[1] = new RowStyle(SizeType.Absolute, S(44));
                headerTbl.Height = S(88);
        ...
        private void OnTC66Disconnect(object sender, EventArgs e)
        {
            ...
            // Hide TC66 row and collapse header
            foreach (Control c in new Control[] { lblTC66Status, lblTC66Volt, lblTC66Amp,
                lblTC66Resistance, lblTC66Power, lblTC66mAh, lblTC66mWh, lblTC66Eff })
                c.Visible = false;
            headerTbl.RowStyles[1] = new RowStyle(SizeType.Absolute, 0);
            headerTbl.Height = S(44);
        ...
        private void UpdateTC66Header(TC66Reading r)
        {
            ...
            lblTC66Volt.Text = "U=" + r.Voltage_V.ToString("F2") + " V";
            ...
            if (r.Current_A > 0.01)
            {
                ...
                lblTC66Resistance.Text = "R=" + tc66Resistance.ToString("F0") + " Ω";
            ...
            lblTC66Power.Text = "P=" + Fmt3(displayTC66Power) + " W";
            lblTC66Power.ForeColor = powerColor;
            
            lblTC66Temp.Visible = false;  // TC66 temp now shown in lblBMSTemp
            ...
            lblTC66mAh.Text = "Qin=" + Fmt3(mAh / 1000.0) + " Ah";
            lblTC66mWh.Text = "Ein=" + Fmt3(mWh / 1000.0) + " Wh";
        }
        ...
        private void UpdateEfficiency()
        {
            ...
            if (einWh > 0.01 && ecWh > 0.01)
            {
                double eff = (ecWh / einWh) * 100.0;
                lblTC66Eff.Text = (eff > 0 && eff <= 150)
                    ? "CE=" + eff.ToString("F1") + "%"
                    : "CE=--";
            }
            else
            ...
            // TC66 temp in separate red label (col 8, row 1)
            if (lastTC66Reading != null && lastTC66Reading.IsValid)
            {
                lblBMSTemp.Text    = "T.TC=" + lastTC66Reading.Temperature_C + "°C";
                lblBMSTemp.Visible = true;
            }
            else
            {
                lblBMSTemp.Visible = false;
            }
            lblTC66Temp.Visible = false;
        }
        ...
        private Button MakeButton(string text, Color color)
        {
            var b = new Button
            {
                ...
                Font = new Font("Segoe UI", SF(19), FontStyle.Bold, GraphicsUnit.Pixel),
        ...
        // ── Charge-complete triple-beep alert ──
        private void StartChargeCompleteAlert()
        {
            StopChargeCompleteAlert();  // Cancel any prior alert
            beepFireCount = 0;
            if (this.InvokeRequired)
                this.Invoke(new Action(() => { btnStopBeep.Visible = true; ctrlBar.PerformLayout(); }));
            else { btnStopBeep.Visible = true; ctrlBar.PerformLayout(); }
            beepTimer = new System.Threading.Timer(OnBeepTimerTick, null, 0, 60000);
        }

        private void StopChargeCompleteAlert()
        {
            if (beepTimer != null) { beepTimer.Dispose(); beepTimer = null; }
            beepFireCount = 0;
            if (this.InvokeRequired)
                this.Invoke(new Action(() => { btnStopBeep.Visible = false; ctrlBar.PerformLayout(); }));
            else { btnStopBeep.Visible = false; ctrlBar.PerformLayout(); }
        }

        private void OnBeepTimerTick(object state)
        {
            if (beepFireCount >= 10) { StopChargeCompleteAlert(); return; }
            beepFireCount++;
            _synth.SpeakAsync("Battery full");
        }

        private static readonly string[] _socWords =
            { "", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten" };

        // ── Low SOC speech (10% down to 1%, every 1% step) ──
        private void CheckLowSOCBeep(double soc)
        {
            int socInt = (int)Math.Ceiling(soc);
            if (socInt <= 10 && socInt >= 1 && socInt < lastSOCBeep)
            {
                lastSOCBeep = socInt;
                string word = _socWords[socInt];
        ...
        private void OnToggleRun(object sender, EventArgs e)
        {
            ...
            lastSOCBeep = 999;    // Reset low-SOC beep threshold
            TakeSample();
        }
        ...
        private void LoadCSVFile(string path)
        {
            try
            {
                ...
                foreach (string line in lines.Skip(dataStartLine))
                {
                    ...
                    if (hasTC66)
                    {
                        ...
                        if (tc66v > 0 || tc66a > 0)
                        {
                            ...
                            // TC66 temp not plotted — shown in header only
                        }
                    }
                ...
                if (allReadings.Count > 0)
                {
                    ...
                    // Sync axis/tick colors to the state of the last row in the file
                    chartVI.CurrentBatteryState = last.State;
                    chartPS.CurrentBatteryState = last.State;
        ...
        private void TakeSample()
        {
            ...
            if (allReadings.Count > 0)
            {
                ...
                // Reset segment only on actual direction change (Charging↔Discharging)
                // Idle readings do NOT reset — BMS briefly goes Idle during taper charging
                if (r.State != BatteryState.Idle && lastSegState != BatteryState.Idle
                    && r.State != lastSegState)
                {
                    segCapacity_mAh = 0;
                    segEnergy_Wh = 0;
                    lastAutoSOC = double.NaN;  // Reset auto-LR baseline on state change
                    if (r.State != BatteryState.Discharging)
                        ClearAutoLR();  // Hide line and ETA when not discharging
                    // Sync TC66 baseline to exact same sample as Ec reset
                    if (r.State == BatteryState.Charging)
                        ResetEfficiencyBaseline();
                }
                // Also reset efficiency baseline on first Charging reading (Idle→Charging at session start)
                if (r.State == BatteryState.Charging && lastSegState == BatteryState.Idle
                    && tc66Reader.IsConnected)
                    ResetEfficiencyBaseline();

                // Charge-complete alert: Charging→Idle transition
                if (prevRawState == BatteryState.Charging && r.State == BatteryState.Idle)
                    StartChargeCompleteAlert();
                // Auto-dismiss alert if power source disconnected (Idle→Discharging while alert active)
                if (beepTimer != null && r.State == BatteryState.Discharging && prevRawState != BatteryState.Discharging)
                    StopChargeCompleteAlert();
                prevRawState = r.State;
            ...
            lblSegCap.Text    = "Qc=" + Fmt3(segCapacity_mAh / 1000.0) + " Ah";
            lblTotCap.Text    = "Qt=" + Fmt3(cumulCapacity_mAh / 1000.0) + " Ah";
            lblSegEnergy.Text = "Ec=" + Fmt3(segEnergy_Wh) + " Wh";
            lblTotEnergy.Text = "Et=" + Fmt3(cumulEnergy_Wh) + " Wh";
            lblSamples.Text = allReadings.Count + " samples";

            // Update efficiency and auto-LR
            if (tc66Reader.IsConnected)
                UpdateEfficiency();
            try { UpdateAutoLR(r); RefreshAutoLRETA(); }
            catch { /* auto-LR error must not kill sample loop */ }
        }
        ...
        // ── Auto-LR: track SOC step-down corners and compute ETA ──────────────────
        private void UpdateAutoLR(BatteryReading r)
        {
            if (r.State != BatteryState.Discharging)
            {
                lblAutoETA.Visible = false;
            ...
            double soc = r.SOC_Percent;
            double elapsed = r.ElapsedSeconds;

            // Detect a step-down ≥ AutoLRMinDrop %
            if (!double.IsNaN(lastAutoSOC) && (lastAutoSOC - soc) > 0)
            {
                autoLRTimes.Add(elapsed);
                autoLRSOC.Add(soc);

                // Primary window: last 5 minutes of step-down points
                // Fallback: if fewer than N points in 5 min window, keep N most recent
                int n = nudLRPoints != null ? (int)nudLRPoints.Value : 5;
                const double windowSec = 5 * 60.0;  // 5-minute window
                // Remove points older than 5 minutes
                while (autoLRTimes.Count > 0 && (elapsed - autoLRTimes[0]) > windowSec)
                { autoLRTimes.RemoveAt(0); autoLRSOC.RemoveAt(0); }
                // Fallback: if fewer than N points remain, keep at least N most recent
                while (autoLRTimes.Count > n) { autoLRTimes.RemoveAt(0); autoLRSOC.RemoveAt(0); }

                // Need at least 3 points for a meaningful fit
                if (autoLRTimes.Count >= 3)
                {
                    // Linear regression: SOC = a + b * elapsed
                    int cnt = autoLRTimes.Count;
                    double sumX = 0, sumY = 0, sumXY = 0, sumX2 = 0;
                    for (int i = 0; i < cnt; i++)
                    {
                        sumX  += autoLRTimes[i];
                        sumY  += autoLRSOC[i];
                        sumXY += autoLRTimes[i] * autoLRSOC[i];
                        sumX2 += autoLRTimes[i] * autoLRTimes[i];
                    }
                    double denom = cnt * sumX2 - sumX * sumX;
                    if (Math.Abs(denom) < 1e-9) { lastAutoSOC = soc; return; }
                    double b = (cnt * sumXY - sumX * sumY) / denom;  // slope (% per sec, negative)
                    double a = (sumY - b * sumX) / cnt;               // intercept
                    ...
                    for (int i = 0; i < cnt; i++)
                    {
                        double yHat = a + b * autoLRTimes[i];
                        ssRes += (autoLRSOC[i] - yHat) * (autoLRSOC[i] - yHat);
                        ssTot += (autoLRSOC[i] - meanY) * (autoLRSOC[i] - meanY);
                    }
                    double r2 = ssTot < 1e-12 ? 0 : 1.0 - ssRes / ssTot;

                    // Predict time when SOC = AutoLRFloor
                    if (Math.Abs(b) < 1e-12) { lastAutoSOC = soc; return; }
                    // Store intercept as absolute time so ETA can be recalculated any time
                    double interceptAbsolute = (AutoLRFloor - a) / b;  // seconds from startTime
                    double etaSeconds = interceptAbsolute - elapsed;

                    bool reliable = r2 >= 0.9;
                    if (etaSeconds > 0)
                    {
                        int totalMin = (int)(etaSeconds / 60);
                        int hh = totalMin / 60, mm = totalMin % 60;
                        string etaStr = (reliable ? "TT5: " : "TT5: ~") +
                                        (hh > 0 ? hh + "h " : "") + mm + "m";
                        lblAutoETA.Text      = etaStr;
                        lblAutoETA.ForeColor = reliable
                            ? Color.FromArgb(180, 0, 180)
                            : Color.FromArgb(150, 150, 150);
                        lblAutoETA.Visible   = true;
                    }
                    else
                    {
                        lblAutoETA.Text    = "TT5: now";
                        lblAutoETA.ForeColor = Color.FromArgb(211, 47, 47);
                        lblAutoETA.Visible = true;
                    }
                    // Expand row 1 if TC66 row is collapsed
                    if (!tc66Reader.IsConnected && headerTbl.RowStyles[1].Height == 0)
                    {
                        headerTbl.RowStyles[1] = new RowStyle(SizeType.Absolute, S(44));
                        headerTbl.Height = S(88);
                    }

                    // Push auto-LR data to chartPS for drawing
                    double intercept = (AutoLRFloor - a) / b;
                    chartPS.SetAutoLR(autoLRTimes, autoLRSOC, a, b, r2, intercept);
                    mfAutoLR_b = b; mfAutoLR_r2 = r2;
                    mfAutoLRIntercept = intercept; mfAutoLRValid = true;
                }
            }
            lastAutoSOC = soc;
        }

        // Called every sample to keep ETA display current without waiting for next step
        private void RefreshAutoLRETA()
        {
            if (!mfAutoLRValid || mfAutoLR_b >= 0) return;
            double elapsed = (DateTime.Now - startTime).TotalSeconds;
            double etaSeconds = mfAutoLRIntercept - elapsed;
            bool reliable = mfAutoLR_r2 >= 0.9;
            if (etaSeconds > 0)
            {
                int totalMin = (int)(etaSeconds / 60);
                int hh = totalMin / 60, mm = totalMin % 60;
            ...
            else if (etaSeconds > -300)  // Up to 5 min past
                lblAutoETA.Text = "TT5: now";
        }

        private void ClearAutoLR()
        {
            autoLRTimes.Clear();
            autoLRSOC.Clear();
            lastAutoSOC = double.NaN;
            mfAutoLRValid = false; mfAutoLR_b = 0; mfAutoLR_r2 = 0; mfAutoLRIntercept = 0;
            lblAutoETA.Visible = false;
            // Collapse row 1 if TC66 disconnected
            if (!tc66Reader.IsConnected)
            {
                headerTbl.RowStyles[1] = new RowStyle(SizeType.Absolute, 0);
                headerTbl.Height = S(44);
        ...
        private void UpdateHeader(BatteryReading r)
        {
            if (r == null)
            {
                ...
                lblVoltage.Text = "U=-- V"; lblVoltage.ForeColor = TextCol;
            ...
            { 
                ...
                currentColor = chartVI.Y2DischargeColor;  // Bright Red
                powerColor = chartPS.Y1DischargeColor;    // Amber
                socColor = chartPS.Y2DischargeColor;      // Pure red (matches T.TC)
            }
            else
            { 
            ...
            lblVoltage.Text = "U=" + (r.Voltage_V > 0 ? r.Voltage_V.ToString("F2") : "--") + " V";
            ...
            {
                double resistance = r.Voltage_V / current_A;
                lblResistance.Text = "R=" + resistance.ToString("F0") + " Ω";
            ...
            lblPower.Text = "P=" + Fmt3(displayPower) + " W";
            lblPower.ForeColor = powerColor;
            
            lblSOC.Text = "SOC=" + r.SOC_Percent.ToString("F1") + " %";
            lblSOC.ForeColor = socColor;
            
            // Health and cycle count (updated from WMI cache)
            if (r.HealthPercent > 0)
            {
                lblHealth.Text = "SoH=" + r.HealthPercent.ToString("F1") + "%";
                lblCycles.Text = "Cyc=" + r.CycleCount;
            }

            // BMS ETA — append to ETA label or show separately
            // (shown via lblAutoETA which already handles LR ETA)

            // Ttc label visibility managed by UpdateEfficiency()
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            if (_headerFontRef  != null) { _headerFontRef.Dispose();  _headerFontRef  = null; }
            DisableStayAwake();
    ...
    static class Program
    {
        ...
        private static extern int SetProcessDpiAwareness(int v);
