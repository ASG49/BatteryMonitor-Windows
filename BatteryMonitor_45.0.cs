//=============================================================================
// BatteryMonitor.cs - Windows Battery Monitor v45.0
...
namespace BatteryMonitor
{
    ...
    public class DualAxisChartPanel : Panel
    {
        ...
        private void ComputeAvg()
        {
            ...
            avgValue = sum / count;
            avgResult = name + " avg = " + avgValue.ToString(Math.Abs(avgValue) < 10.0 ? "F2" : "F1") + " " + unit;
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            int mL = S(100), mR = S(100), mT = S(48), mB = S(95);  // Scale margins with DPI
            ...
            if (yData1.Count > 0)
            {
                ...
                string fmt = ChartTitle.Contains("Voltage") ? "F2" : "F2";
                ...
                if (ChartTitle.Contains("Power") && xDataEnergySegment.Count > 0)
                {
                    ...
                    DrawValueBadgeAtY(g, pa, ecVal, "Wh", ecColor, true, Math.Abs(ecVal) < 10.0 ? "F2" : "F1", 38);
                }
            }
            if (yData2.Count > 0)
            {
        ...
        {
            using (var font = new Font("Segoe UI", ScaleFont(22f), FontStyle.Regular, GraphicsUnit.Pixel))  // Axis titles
            ...
            double y2Min, double y2Span)
        {
            using (var font = new Font("Segoe UI", ScaleFont(17f), FontStyle.Regular, GraphicsUnit.Pixel))
            {
                ...
                // TC66 Temperature
                if (xDataTemp.Count > 0)
                    entries.Add(new LegendEntry("T.TC, °C", TempColor, 0.5f, true));
            ...
            double y2Low, double y2Span)
        {
            ...
            {
                lines.Add(Y1Title + ": " + yData1[bestIdx1].ToString(Y1Unit == "V" ? "F2" : (Math.Abs(yData1[bestIdx1]) < 10.0 ? "F2" : "F1")) + " " + Y1Unit);
                curveColor = y1Col;
            }
            ...
            {
                lines.Add("TC66 " + Y1Title + ": " + yDataTC66_1[bestIdxTC66_1].ToString(Y1Unit == "V" ? "F2" : (Math.Abs(yDataTC66_1[bestIdxTC66_1]) < 10.0 ? "F2" : "F1")) + " " + Y1Unit);
            ...
            {
                lines.Add("Ec (Wh): " + yDataEnergySegment[bestIdxEnergySegment].ToString(Math.Abs(yDataEnergySegment[bestIdxEnergySegment]) < 10.0 ? "F2" : "F1"));
                curveColor = Color.FromArgb(0xBA, 0x68, 0xC8);
            }
            else if (closestCurve == "EnergyTotal" && bestIdxEnergyTotal >= 0)
            {
                lines.Add("Et (Wh): " + yDataEnergyTotal[bestIdxEnergyTotal].ToString(Math.Abs(yDataEnergyTotal[bestIdxEnergyTotal]) < 10.0 ? "F2" : "F1"));
    ...
    {
        private const string AppVersion = "v45.0";  // ← UPDATE ONLY HERE
        ...
        private void BuildUI()
        {
            ...
            {
                Text = "\u2715 Beep", Width = BW("\u2715 Beep"), Height = btnHeight,
            ...
            chartVI = new DualAxisChartPanel
            {
                ...
                Y1Title = "U, V", Y1Unit = "V",
                Y2Title = "i, A", Y2Unit = "A",
        ...
        private void UpdateTC66Header(TC66Reading r)
        {
            ...
            lblTC66Power.Text = "P=" + displayTC66Power.ToString(Math.Abs(displayTC66Power) < 10.0 ? "F2" : "F1") + " W";
            ...
            lblTC66mWh.Text = "Ein=" + (mWh / 1000.0).ToString(Math.Abs(mWh / 1000.0) < 10.0 ? "F2" : "F1") + " Wh";
        }
        ...
        private void OnBeepTimerTick(object state)
        {
            ...
            System.Threading.ThreadPool.QueueUserWorkItem(_ =>
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (i < 2) System.Threading.Thread.Sleep(120);
                }
            });
        }

        // ── Low SOC single beep (10% down to 1%, every 1% step) ──
        private void CheckLowSOCBeep(double soc)
        {
            int socInt = (int)soc;
            if (socInt <= 10 && socInt < lastSOCBeep)
            {
                lastSOCBeep = socInt;
                System.Threading.ThreadPool.QueueUserWorkItem(_ => Console.Beep(660, 200));
            }
        }
        ...
        private void TakeSample()
        {
            ...
            lblSegEnergy.Text = "Ec=" + segEnergy_Wh.ToString(Math.Abs(segEnergy_Wh) < 10.0 ? "F2" : "F1") + " Wh";
            lblTotEnergy.Text = "Et=" + cumulEnergy_Wh.ToString(Math.Abs(cumulEnergy_Wh) < 10.0 ? "F2" : "F1") + " Wh";
        ...
        private void UpdateHeader(BatteryReading r)
        {
            ...
            lblPower.Text = "P=" + displayPower.ToString(Math.Abs(displayPower) < 10.0 ? "F2" : "F1") + " W";
