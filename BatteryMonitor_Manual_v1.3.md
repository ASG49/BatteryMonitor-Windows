...
> **Long-session note:** BatteryMonitor uses LTTB (Largest Triangle Three Buckets) decimation to cap rendered points at 10,000 per curve regardless of session length. Raw data is always preserved in full for CSV export, tooltip, and regression — only the drawn curves are decimated. This prevents UI slowdown during multi-day sessions of 200,000+ samples.
...
2. Run the downloaded `BatteryMonitor_46.0.exe` directly — no installer or ZIP extraction required
