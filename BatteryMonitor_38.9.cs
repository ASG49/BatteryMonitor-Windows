//=============================================================================
// BatteryMonitor.cs - Windows Battery Monitor v38.9
...
namespace BatteryMonitor
{
    ...
    public class DualAxisChartPanel : Panel
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (y2Range > 0)
            {
                ...
                if (Y2Unit == "%")
                {
                    // SOC: tight padding, do NOT floor at 0 (wastes chart space)
                    visY2Min -= y2Pad;
                    visY2Max += y2Pad;
                    visY2Max = Math.Min(visY2Max, 103);
                }
            ...
            if (Y2Unit == "%")
            {
                // SOC: cap at 100%, floor at nearest tick below data min (not 0)
                y2AxisMax = Math.Min(y2AxisMax, 100);
                // Keep floor at rounded tick min — do NOT force 0 for SOC
                ...
                && CurrentBatteryState == BatteryState.Discharging)
            {
                ...
                // Left anchor: regression line at first corner point or left chart edge, whichever is later
                double alrXStart = Math.Max(autoLRTimes[0], xFirst);
                double alrYStart = autoLR_a + autoLR_b * alrXStart;
                double alrPxLf = pa.Left + (alrXStart - xFirst) / xRange * pa.Width;
                double alrPyLf = pa.Bottom - (alrYStart - y2AxisMin) / y2Span * pa.Height;
    ...
    {
        private const string AppVersion = "v38.9";  // ← UPDATE ONLY HERE
