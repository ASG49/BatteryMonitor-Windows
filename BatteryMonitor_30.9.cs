//=============================================================================
// BatteryMonitor.cs - Windows Battery Monitor v30.9
// May 19, 2026 - Tony Gozdz
//
...
//
// Key Features v30.9:
//   - CSV Timestamp field quoted ("2026-05-19 13:45:52") — prevents Excel splitting on the space
...
namespace BatteryMonitor
{
    ...
    public class TC66Reader : IDisposable
    {
        ...
        public TC66Reading Poll()
        {
            ...
            try
            {
                ...
                {
                    decryptor.TransformBlock(encrypted, 0, 192, decrypted, 0);
                }
    ...
    public class DualAxisChartPanel : Panel
    {
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            // Check for third click to clear avg
            if (avgClickState == 2)
            {
                ClearAvg();
                return;
            ...
            // Check for third click to clear LR
            ...
            // Find nearest point across all clickable traces
            int nearestTrace = 0;  // 1=Y1, 2=Y2, 5=TC66_1, 6=TC66_2
            int nearestIdx = -1;
            ...
            // Check Y2 for LR or avg
            if ((EnableLR || EnableAvgY2) && xData2.Count >= 2)
            {
                int idx; double dist;
                FindNearestPoint(xData2, yData2, py2Low, py2Span, e.X, e.Y, out idx, out dist);
                if (idx >= 0 && dist < nearestDist) { nearestDist = dist; nearestIdx = idx; nearestTrace = 2; }
            }

            // Check TC66_1 for avg
            ...
            // Handle LR on Y2 (SOC) if LR enabled and Y2 is nearest
            ...
            else if (EnableLR && nearestTrace == 2)
            {
                HandleLRClick(nearestIdx);
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                using (var font = new Font("Segoe UI", ScaleFont(14f)))
            ...
            // Include TC66 data in min/max
            for (int i = 0; i < xDataTC66_1.Count; i++)
            {
                if (xDataTC66_1[i] >= xFirst)
                {
            ...
            using (var brush = new SolidBrush(y1TickCol))
            {
                for (double v = y1tMin; v <= y1tMax + y1tStep * 0.01; v += y1tStep)
                {
                    ...
                    g.DrawString(lbl, font, brush, pa.Left - sz.Width - 6, py - sz.Height / 2);
                }
            ...
            // X axis title
            using (var font = new Font("Segoe UI", ScaleFont(13f)))  // Axis title
            using (var brush = new SolidBrush(TextColor))
            {
            ...
            if (EnableLR && lrClickState >= 1)
            {
                ...
                if (lrClickState == 2)
                {
                    ...
                    // LR prediction text inside chart (positioned below legend)
                    if (lrPrediction.Length > 0)
                    {
                        ...
                        using (var brush = new SolidBrush(LRLineColor))
                        {
                            ...
                            // Position below legend: legend is at pa.Top + 4, with height ~25px, so start at ~35
                            float ty = pa.Top + 35;
                            ...
                            // Draw total run time (+ total session prediction) below LR prediction
                            if (RunTimeSeconds > 0)
                            {
                                ...
                                // Append Tot: if a valid LR prediction exists
                                ...
                                using (var timeFont = new Font("Segoe UI", ScaleFont(15f), FontStyle.Bold))
                                using (var timeBrush = new SolidBrush(Color.FromArgb(100, 100, 100)))
                                {
                                    ...
                                    float timeTy = ty + sz.Height + 4;  // 4px gap below LR text
            ...
            if (avgClickState >= 1)
            {
                ...
                if (avgClickState == 2)
                {
                    ...
                    // Draw horizontal line at average value
                    ...
                    // Average result text inside chart
                    if (avgResult.Length > 0)
                    {
                        using (var font = new Font("Segoe UI", ScaleFont(12f), FontStyle.Bold))
                        using (var brush = new SolidBrush(avgColor))
                        {
                            SizeF sz = g.MeasureString(avgResult, font);
                            float tx = pa.Left + (pa.Width - sz.Width) / 2;
                            float ty = pa.Top + 65;  // Below LR text (which is at pa.Top + 35)
                            using (var bgBr = new SolidBrush(Color.FromArgb(210, 255, 255, 255)))
                                g.FillRectangle(bgBr, tx - 4, ty - 2, sz.Width + 8, sz.Height + 4);
            ...
            if (yData2.Count > 0)
            {
                ...
                if (yDataTemp.Count > 0)
                {
                    ...
                    using (var brush = new SolidBrush(TempColor))
                    {
                        ...
                        float y = pa.Bottom - sz.Height - 38;  // Position above SOC badge
                ...
                if (yDataBMSTemp.Count > 0)
                {
                    ...
                    using (var brush = new SolidBrush(BMSTempColor))
                    {
                        ...
                        SizeF sz = g.MeasureString(txt, font);
                        float x = pa.Right - sz.Width - 8;
                        float y = pa.Bottom - sz.Height - 70;  // Below TC66 temp (which is at -38)
        ...
        {
            using (var font = new Font("Segoe UI", ScaleFont(13f)))  // Axis titles
        ...
        {
            using (var font = new Font("Segoe UI", ScaleFont(10f), FontStyle.Regular))
            {
                int sw = 30;  // Sample line width
                int gap = 12;  // More gap between line sample and text
                int lineHeight = 20;  // Reduced for smaller 10pt font (was 28 for 14pt)
                ...
                // Auto-position legend in emptiest area of chart (with fallback to bottom-center)
                float boxX, boxY;
                try
                {
                ...
                {
                    // Fallback to bottom-center if auto-positioning fails
                ...
                foreach (LegendEntry entry in entries)
                {
                    ...
                    // Draw text (vertically centered in lineHeight)
            ...
            double val, string unit, Color color, bool leftSide, string format, float bottomOffset)
        {
            ...
            using (var brush = new SolidBrush(color))
            {
                ...
                SizeF sz = g.MeasureString(txt, font);
                float x = leftSide ? pa.Left + 8 : pa.Right - sz.Width - 8;
                // Exact same formula as TC66 temp above SOC: pa.Bottom - sz.Height - 38
                float y = pa.Bottom - sz.Height - 38;
            ...
            double y2Low, double y2Span)
        {
            ...
            // Draw tooltip box with larger font (16pt)
            using (var font = new Font("Segoe UI", ScaleFont(14f), FontStyle.Bold))
            {
                float maxWidth = 0;
                float totalHeight = 0;
    ...
    public class MainForm : Form
    {
        ...
        private double bmsBaseEnergy_Wh = 0;   // Starting BMS energy for efficiency calc
        private bool tc66EfficiencyActive = false;
        ...
        {
            this.Text = "Battery Monitor v30.9";
        ...
        {
            return (float)(baseValue * screenScale);
        }
        ...
        private void BuildUI()
        {
            ...
            foreach (var lbl in new[] { lblStatus, lblVoltage, lblCurrent, lblResistance,
                                         lblPower, lblSOC, lblSegCap, lblSegEnergy,
                                         lblTotCap, lblTotEnergy })
            ...
            foreach (var lbl in new[] { lblTC66Status, lblTC66Volt, lblTC66Amp, lblTC66Resistance,
                                         lblTC66Power, lblTC66Temp, lblTC66mAh, lblTC66mWh, lblTC66Eff })
            ...
            Font controlFont = new Font("Segoe UI", SF(11), FontStyle.Bold);  // Reduced from 13
            ...
            btnTC66ScreenFlip = new Button
            {
                ...
                Font = new Font("Segoe UI", SF(15), FontStyle.Bold),
            ...
            lblElapsed = new Label
            {
                ...
                Font = new Font("Consolas", SF(14), FontStyle.Bold),
        ...
        private Label MakeHeaderLabel(string text, Color color)
        {
            ...
            int baseFontSize = (dpiScale <= 1.0f) ? 12 : 14;
        ...
        private void OnTC66Connect(object sender, EventArgs e)
        {
            ...
            if (tc66Reader.Connect(portName))
            {
                ...
                if (r.IsValid)
                {
                    lastTC66Reading = r;
                    UpdateTC66Header(r);
                    lblTC66Status.Text = "TC66 " + r.FirmwareVersion;
                }
        ...
        private void StartCsvRecording()
        {
            ...
            try
            {
                ...
                this.Text = "Battery Monitor v30.9  —  " + fn;
            }
            catch (Exception ex)
            {
        ...
        private void UpdateTC66Header(TC66Reading r)
        {
            ...
            lblTC66Power.Text = "P=" + displayTC66Power.ToString("F2") + " W";
        ...
        private void ResetEfficiencyBaseline()
        {
            if (tc66Reader.IsConnected && lastTC66Reading != null && lastTC66Reading.IsValid)
            {
                ...
                bmsBaseEnergy_Wh = cumulEnergy_Wh;
                efficiencyStartTime = DateTime.Now;
                tc66EfficiencyActive = true;
            }
            // If TC66 connected but lastTC66Reading not yet available, leave tc66EfficiencyActive=false
        ...
        private void UpdateEfficiency()
        {
            ...
            // Auto-activate efficiency tracking on first valid TC66 reading if not yet active
            if (!tc66EfficiencyActive && tc66Reader.IsConnected && isRunning)
            {
                int mWh0 = lastTC66Reading.Group0_mWh != 0 ? lastTC66Reading.Group0_mWh : lastTC66Reading.Group1_mWh;
                tc66BaseEnergy_mWh = mWh0;
                bmsBaseEnergy_Wh = cumulEnergy_Wh;
                efficiencyStartTime = DateTime.Now;
                tc66EfficiencyActive = true;
                lblTC66Eff.Text = "η=--%";
                return;  // Return now; need at least one more reading to compute delta
            }

            if (!tc66EfficiencyActive)
            {
                lblTC66Eff.Text = "η=--%";
                return;
            }
            
            // Use whichever group has non-zero values (TC66 can use Group0 or Group1)
            int currentMWh = lastTC66Reading.Group0_mWh != 0 ? lastTC66Reading.Group0_mWh : lastTC66Reading.Group1_mWh;
            double tc66DeltaWh = (currentMWh - tc66BaseEnergy_mWh) / 1000.0;
            double bmsDeltaWh = cumulEnergy_Wh - bmsBaseEnergy_Wh;
            
            // Subtract system baseline power consumption if specified
            double psys = (double)nudPsys.Value;
            if (psys > 0)
            {
                double elapsedHours = (DateTime.Now - efficiencyStartTime).TotalHours;
                double systemEnergyWh = psys * elapsedHours;
                tc66DeltaWh -= systemEnergyWh;
            }
            
            if (tc66DeltaWh > 0.001)  // Need at least 1mWh of corrected input
            {
                double eff = (bmsDeltaWh / tc66DeltaWh) * 100.0;
                if (eff > 0 && eff <= 150)  // Allow up to 150% for measurement tolerance
        ...
        private Button MakeButton(string text, Color color)
        {
            var b = new Button
            {
                ...
                Font = new Font("Segoe UI", SF(14), FontStyle.Bold),
        ...
        private void LoadCSVFile(string path)
        {
            try
            {
                ...
                if (ParentForm != null) ParentForm.Text = "Battery Monitor v30.9  —  " + shortName;
        ...
        private void UpdateHeader(BatteryReading r)
        {
            ...
            lblPower.Text = "P=" + displayPower.ToString("F1") + " W";
