...
- **Charge complete:** when charging current drops to zero (CHRG→IDLE transition), a triple beep (880 Hz) sounds every 60 seconds for up to 10 minutes, signalling that a discharge cycle can be started. Dismissed by the red **✕ Beep** button that appears on the right of the footer.
...
2. Extract the ZIP — no installer required
3. Ensure `app.manifest` is in the same folder as `BatteryMonitor.exe` (it is included in the release)
